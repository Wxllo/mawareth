...
export const renovationsApi = {
  /**
   * Get renovation offer for estate
   */
  getByEstateId: async (estateId: string): Promise<ApiResponse<RenovationOffer>> => {
    try {
      const response = await apiClient.get<ApiResponse<RenovationOffer>>(`/renovations/estate/${estateId}`);
      return response.data;
    } catch (error: any) {
      return {
        success: false,
        error: error.response?.data?.error || 'Failed to fetch renovation offer',
      };
    }
  },
   ...
   */
  apply: async (estateId: string): Promise<ApiResponse<RenovationOffer>> => {
    try {
      const response = await apiClient.post<ApiResponse<RenovationOffer>>(`/renovations/apply`, { estateId });
      return response.data;
    } catch (error: any) {
      return {
        success: false,
        error: error.response?.data?.error || 'Failed to apply for renovation',
      };
    }
  },
};

...
import { mockUsers, mockEstates, mockMarketplaceListings, mockRenovationOffers } from '@/data/mockData';
...
export const api = {
  ...
  // Renovations
  renovations: {
    getByEstateId: async (estateId: string): Promise<RenovationOffer | undefined> => {
      await delay(200);
      return mockRenovationOffers.find(r => r.estateId === estateId);
    },
    apply: async (estateId: string): Promise<boolean> => {
      await delay(400);
      console.log(`Renovation application submitted for estate ${estateId}`);
      return true;
    },
  },
};
